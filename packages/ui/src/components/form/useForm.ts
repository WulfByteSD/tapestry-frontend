...
export type FieldValidator<T extends FormValues, K extends FieldName<T>> = (
  value: T[K],
  values: T,
) => string | undefined;
...
export type FormFieldApi<T extends FormValues, K extends FieldName<T>> = {
  ...
  onChange: (
    eventOrValue: T[K] | React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>,
  ) => void;
...
function toValidatorArray<T extends FormValues, K extends FieldName<T>>(
  validator?: FormValidators<T>[K],
): Array<FieldValidator<T, K>> {
  if (!validator) return [];
  return Array.isArray(validator) ? validator : [validator];
}

function getFieldError<T extends FormValues, K extends FieldName<T>>(
  ...
  const setValue = React.useCallback(
    <K extends FieldName<T>>(name: K, value: T[K], options?: SetFieldValueOptions) => {
      ...
      const shouldValidate =
        options?.validate ?? (validateOnChange || Boolean(touchedRef.current[name]) || submitCountRef.current > 0);
